...
public class WechatResultActivity extends BaseActivity {
            ...
            GUIAction guiAction = new GUIAction() {
                protected void doAction(Object[] paramObjs) {
                    ...
                    if (remainedSeconds == 0) {
                        ...
                        if (!StringUtils.isBlank(WechatResultActivity.this.JSON) && "BOTTLE".equalsIgnoreCase(ServiceGlobal.getCurrentSession("PRODUCT_TYPE").toString())) {
                            intent.setClass(WechatResultActivity.this, EnvironmentalPromotionalActivity.class);
                            intent.putExtra("JSON", WechatResultActivity.this.JSON);
                        } else if (!StringUtils.isBlank(SysConfig.get("RVMMActivity.class"))) {
                            try {
                                intent.setClass(WechatResultActivity.this.getBaseContext(), Class.forName(SysConfig.get("RVMMActivity.class")));
                            } catch (Exception e) {
                                e.printStackTrace();
                            }
                        }
                        intent.setFlags(FLAG_ACTIVITY_REORDER_TO_FRONT);
                        WechatResultActivity.this.startActivity(intent);
                        WechatResultActivity.this.finish();
                        return;
                    }
                    ((TextView) WechatResultActivity.this.findViewById(R.id.wechat_rebate_time)).setText("" + remainedSeconds);
                }
            };
            WechatResultActivity.this.executeGUIAction(false, guiAction, new Object[]{Integer.valueOf(forwardSeconds), Integer.valueOf(remainedSeconds)});
        }
    };
        ...
        try {
            ...
            if (hsmpStatus != null) {
                ...
                if (CARD_STATUS == -1 || CARD_STATUS == -2) {
                    this.recharge_warningtext.setText(R.string.wechat_unuse);
                    this.wechatRebateInfoLayout.setVisibility(8);
                } else {
                    if (this.isPlaySounds && this.soundPool != null) {
                        ...
                        this.soundPool.setOnLoadCompleteListener(new OnLoadCompleteListener() {
                            public void onLoadComplete(SoundPool soundPool, int sampleId, int status) {
                                if (sampleId == WechatResultActivity.this.fanLiWanCheng) {
                ...
                this.recharge_warningtext.setText(R.string.commit_rebateinfo_fail_text1);
                this.wechatRebateInfoLayout.setVisibility(8);
            }
    ...
    public void initView() {
        this.rebateThis = (TextView) findViewById(R.id.wechat_rebate_this);
        this.balanceOfINCOM = (TextView) findViewById(R.id.wechat_INCOM_balance);
        this.recharge_warningtext = (TextView) findViewById(R.id.wechat_rebate_remind_text);
        this.wechatRebateInfoLayout = findViewById(R.id.wechat_info);
    }
