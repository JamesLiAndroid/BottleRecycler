...
public class NewOneCardRechargeWarnActivity extends BaseActivity {
    private String JSON;
            ...
            GUIAction guiAction = new GUIAction() {
                protected void doAction(Object[] paramObjs) {
                    ...
                    if (remainedSeconds == 0) {
                        ...
                        if (!StringUtils.isBlank(NewOneCardRechargeWarnActivity.this.JSON) && "BOTTLE".equalsIgnoreCase(ServiceGlobal.getCurrentSession("PRODUCT_TYPE").toString())) {
                            intent.setClass(NewOneCardRechargeWarnActivity.this, EnvironmentalPromotionalActivity.class);
                            intent.putExtra("JSON", NewOneCardRechargeWarnActivity.this.JSON);
                        } else if (!StringUtils.isBlank(SysConfig.get("RVMMActivity.class"))) {
                            try {
                                intent.setClass(NewOneCardRechargeWarnActivity.this.getBaseContext(), Class.forName(SysConfig.get("RVMMActivity.class")));
                            } catch (Exception e) {
                                e.printStackTrace();
                            }
                        }
                        intent.setFlags(FLAG_ACTIVITY_REORDER_TO_FRONT);
                        NewOneCardRechargeWarnActivity.this.startActivity(intent);
                        NewOneCardRechargeWarnActivity.this.finish();
                        return;
                    }
                    ((TextView) NewOneCardRechargeWarnActivity.this.findViewById(R.id.onecard_rebate_warn_time)).setText("" + remainedSeconds);
                }
            };
            NewOneCardRechargeWarnActivity.this.executeGUIAction(false, guiAction, new Object[]{Integer.valueOf(forwardSeconds), Integer.valueOf(remainedSeconds)});
        }
    };
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_newonecardrecharge_warning);
        ...
        ((Button) findViewById(R.id.onecard_rebate_warn_confirm)).setOnClickListener(new OnClickListener() {
            public void onClick(View arg0) {
                if (CardEntity.IS_RECHANGE == 0) {
                    remindText.setText(NewOneCardRechargeWarnActivity.this.getString(R.string.recharge_lack_money));
                } else if (!NewOneCardRechargeWarnActivity.this.isClick) {
                    NewOneCardRechargeWarnActivity.this.isClick = true;
                    Intent intent = new Intent(NewOneCardRechargeWarnActivity.this, NewRechargingActivity.class);
                    intent.putExtra("JSON", NewOneCardRechargeWarnActivity.this.JSON);
                    intent.setFlags(FLAG_ACTIVITY_REORDER_TO_FRONT);
                    NewOneCardRechargeWarnActivity.this.startActivity(intent);
                    NewOneCardRechargeWarnActivity.this.finish();
            ...
            if (hashMapQueryOneCardBalance != null) {
                ...
                if (!StringUtils.isBlank(oneCardBalance)) {
                    if (Pattern.compile(SysConfig.get("VERIFY.MONEYINSIDECARD.PATTERN")).matcher(oneCardBalance).matches()) {
                        moneyInsideCard.setText(df.format((double) ((Float.parseFloat(oneCardBalance) / 100.0f) * ((float) Integer.parseInt(SysConfig.get("LOCAL_EXCHANGE_RATE"))))));
                    } else {
    ...
    protected void onStop() {
        ...
        ((TextView) findViewById(R.id.onecard_rebate_warn_time)).setText("");
