...
public class PrinterRDW32ExCommEntity extends BasePrinter implements CommEntity {
    ...
    private Thread printThread = new Thread() {
        public void run() {
            while (true) {
                PrinterCmdExecutorInfo printerCmdExecutorInfo = (PrinterCmdExecutorInfo) PrinterRDW32ExCommEntity.this.printFIFOQueue.pop();
    ...
    private PrinterCmdExecutor printerCmdExecutorPaperState = new PrinterCmdExecutor() {
        public boolean execute(String str, HashMap<String, String> hashMap) {
            PrinterRDW32ExCommEntity.this.execPaperState();
            return true;
        }
    };
    ...
    private class InitThread extends Thread {
        ...
        public void run() {
            PrinterRDW32ExCommEntity.this.checkPaperState();
        }
    }
    ...
    private class RecvThread extends Thread {
        ...
        public void run() {
            InputStream is = PrinterRDW32ExCommEntity.this.printerSerialPort.getInputStream();
            ...
            while (is != null) {
                try {
                    ...
                    if (this.recvByte != buffer[0]) {
                        ...
                        PrinterRDW32ExCommEntity.logger.debug("Printer State Changed:" + EncryptUtils.byte2hex(buffer, 0, 1));
                    }
                    if (PrinterRDW32ExCommEntity.this.logRecvEnable) {
                        PrinterRDW32ExCommEntity.logger.debug("Printer RECV:" + EncryptUtils.byte2hex(buffer, 0, 1));
                    }
                    PrinterRDW32ExCommEntity.this.errorTimes = 0;
                    PrinterRDW32ExCommEntity.this.monitorError();
                    PrinterRDW32ExCommEntity.this.printerFIFOQueue.push(new Byte(buffer[0]));
                } catch (Exception e) {
                    PrinterRDW32ExCommEntity.this.printerSerialPort.close();
                    PrinterRDW32ExCommEntity.this.printerSerialPort = null;
                    return;
                } catch (Throwable th) {
                    PrinterRDW32ExCommEntity.this.printerSerialPort.close();
                    PrinterRDW32ExCommEntity.this.printerSerialPort = null;
    ...
    public PrinterRDW32ExCommEntity(int id) {
        this.id = id;
        ...
        synchronized (this) {
            try {
                if (this.printerSerialPort == null) {
                    String baudString = SysConfig.get("COM.PRINTER" + this.id + ".BAND");
                    if (!StringUtils.isBlank(baudString)) {
                        if (baudString.indexOf(",") >= 0) {
                            String[] listBaud = baudString.split(",");
                            for (int i = 0; i < listBaud.length; i++) {
                                if (!StringUtils.isBlank(listBaud[i])) {
                                    close();
                                    int oneBaud = Integer.parseInt(listBaud[i].trim());
                                    this.printerSerialPort = new SerialPort(new File(SysConfig.get("COM.PRINTER" + this.id + "." + SysConfig.get("PLATFORM"))), Integer.valueOf(listBaud[i]).intValue(), 0);
                                    if (checkComm(this.printerSerialPort.getInputStream(), this.printerSerialPort.getOutputStream())) {
                                        this.baud = Integer.valueOf(listBaud[i]).intValue();
                                        break;
                                    }
                                }
                            }
                        } else {
                            this.baud = Integer.valueOf(baudString).intValue();
                            this.printerSerialPort = new SerialPort(new File(SysConfig.get("COM.PRINTER" + this.id + "." + SysConfig.get("PLATFORM"))), Integer.valueOf(baudString).intValue(), 0);
                        }
    ...
    private boolean checkComm(InputStream is, OutputStream os) {
        try {
            byte[] cmdPkg = new byte[]{(byte) 27, (byte) 118};
            int len = 0;
            for (int i = 0; i < 2; i++) {
                os.write(cmdPkg);
                Thread.sleep(5000);
    ...
    public void close() {
        synchronized (this) {
            if (this.printerSerialPort != null) {
                this.printerSerialPort.close();
                this.printerSerialPort = null;
            }
        }
    }
    ...
    private void clearPrinterBuffer() {
        this.printerFIFOQueue.reset();
        if (sendData(new byte[]{(byte) 27, (byte) 118})) {
    ...
    private void execPaperState() {
        if (this.checkStateEnable) {
            byte[] bufferState = new byte[]{(byte) 27, (byte) 118};
            long lStartTime = System.currentTimeMillis();
            if (sendData(bufferState)) {
                ...
                if ((((Byte) obj).byteValue() & 1) == 1) {
                    ...
                    if (this.paperState != HardwareAlarmState.NORMAL) {
                        ...
                        logger.debug("Alarm recovery;CMD:" + EncryptUtils.byte2hex(bufferState) + ";RECV:" + EncryptUtils.byte2hex(new byte[]{obj.byteValue()}));
                        return;
                    }
                    return;
                }
    ...
    L_0x00a5:
        ...
        throw r13;
        */
        throw new UnsupportedOperationException("Method not decompiled: com.incomrecycle.prms.rvm.service.comm.entity.PrinterRDW32ExCommEntity.printImage(java.lang.String, java.lang.String, java.lang.String, java.lang.String):boolean");
    }
