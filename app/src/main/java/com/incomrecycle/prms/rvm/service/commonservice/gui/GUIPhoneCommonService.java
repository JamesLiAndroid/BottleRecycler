...
public class GUIPhoneCommonService implements AppCommonService {
    public HashMap execute(String svcName, String subSvcName, HashMap hsmpParam) throws Exception {
        ...
        if ("phoneMovieTicket".equalsIgnoreCase(subSvcName)) {
            return phoneMovieTicket(svcName, subSvcName, hsmpParam);
        }
        return null;
    }

    private HashMap phoneMovieTicket(String svcName, String subSvcName, HashMap hsmpParam) throws Exception {
        HashMap hsmpYoukuParam = new HashMap();
        hsmpYoukuParam.put("CARD_NO", ServiceGlobal.getCurrentSession("PHONE_NO"));
        hsmpYoukuParam.put("CARD_TYPE", "PHONE");
        HashMap hsmpMovieTicketResult = new GUIRecycleCommonService().execute("GUIRecycleCommonService", "youkuMovieTicket", hsmpYoukuParam);
        HashMap haspRet = new HashMap();
        if (hsmpMovieTicketResult == null) {
            return null;
        }
        if ("success".equalsIgnoreCase((String) hsmpMovieTicketResult.get("RESULT"))) {
            haspRet.put("RESULT", NetworkUtils.NET_STATE_SUCCESS);
        }
        if ("error".equalsIgnoreCase((String) hsmpMovieTicketResult.get("RESULT"))) {
            haspRet.put("RESULT", "ERROR");
        }
        if (!"failed".equalsIgnoreCase((String) hsmpMovieTicketResult.get("RESULT"))) {
            return haspRet;
        }
        haspRet.put("RESULT", NetworkUtils.NET_STATE_FAILED);
        return haspRet;
    }

    private HashMap recycleEnd(String svcName, String subSvcName, HashMap hsmpParam) {
        try {
            String PHONE_NO = (String) ServiceGlobal.getCurrentSession("PHONE_NO");
            String CARD_TYPE = "PHONE";
            ...
            if (TRANSMIT_ADV == null || TRANSMIT_ADV.size() <= 0) {
                ...
                if (mapVendingWayFlag != null && mapVendingWayFlag.size() > 0) {
                    ...
                    if (VENDING_WAY_SET == null || !VENDING_WAY_SET.contains("PHONE")) {
                        String VENDING_WAY = (String) mapVendingWayFlag.get(AllAdvertisement.VENDING_WAY);
                        if (VENDING_WAY != null && "PHONE".equalsIgnoreCase(VENDING_WAY)) {
            ...
            sqlUpdateBuilder.setString(AllAdvertisement.VENDING_WAY, "PHONE").setNumber("OPT_STATUS", Integer.valueOf(1)).setString("CARD_TYPE", CARD_TYPE).setString("CARD_NO", PHONE_NO).setString(AllAdvertisement.SELECT_FLAG, SELECT_FLAG).setSqlWhere(sqlWhereBuilderRvmOpt);
            SQLiteExecutor.execSql(sqliteDatabase, sqlUpdateBuilder.toSql());
            String CARD_NO = (String) ServiceGlobal.getCurrentSession("PHONE_NO");
    ...
    private HashMap verifyPhone(String svcName, String subSvcName, HashMap hsmpParam) {
        HashMap hsmpResult = new HashMap();
        ...
        if (StringUtils.isBlank(PHONE_NO)) {
            ...
            if (!StringUtils.isBlank(PHONE_NO)) {
                String VERIFY_PHONE = SysConfig.get("VERIFY.PHONE.PATTERN");
                String VERIFY_CMCC_PHONE = SysConfig.get("VERIFY.CMCC.PHONE");
                if (PHONE_NO.matches(VERIFY_PHONE)) {
                    ServiceGlobal.setCurrentSession("CARD_NO", PHONE_NO);
                    ServiceGlobal.setCurrentSession("PHONE_NO", PHONE_NO);
                    ServiceGlobal.setCurrentSession("CARD_TYPE", "PHONE");
                    if (PHONE_NO.matches(VERIFY_CMCC_PHONE)) {
                        hsmpResult.put("RET_PHONE_NO", "cmcc_phone_num");
                    } else {
                        hsmpResult.put("RET_PHONE_NO", "right_phone_num");
                    }
                }
            }
