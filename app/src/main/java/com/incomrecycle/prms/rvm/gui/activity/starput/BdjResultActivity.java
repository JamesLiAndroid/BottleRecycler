...
public class BdjResultActivity extends BaseActivity {
    private String JSON;
    private int TODAY_BOTTLE = 0;
    private TextView balanceOfINCOM = null;
    private View bdjRebateFailedLayout = null;
    private View bdjRebateInfoLayout = null;
    private TextView bdj_tv_show;
    private int fanLiWanCheng;
    private boolean isPlaySounds;
    private ImageView iv_erweima;
    private TextView rebateThis = null;
    private TextView rechargeFailedText = null;
            ...
            GUIAction guiAction = new GUIAction() {
                protected void doAction(Object[] paramObjs) {
                    ...
                    if (remainedSeconds == 0) {
                        ...
                        if (!StringUtils.isBlank(BdjResultActivity.this.JSON) && "BOTTLE".equalsIgnoreCase(ServiceGlobal.getCurrentSession("PRODUCT_TYPE").toString())) {
                            intent.setClass(BdjResultActivity.this, EnvironmentalPromotionalActivity.class);
                            intent.putExtra("JSON", BdjResultActivity.this.JSON);
                        } else if (!StringUtils.isBlank(SysConfig.get("RVMMActivity.class"))) {
                            try {
                                intent.setClass(BdjResultActivity.this.getBaseContext(), Class.forName(SysConfig.get("RVMMActivity.class")));
                            } catch (Exception e) {
                                e.printStackTrace();
                            }
                        }
                        intent.setFlags(FLAG_ACTIVITY_REORDER_TO_FRONT);
                        BdjResultActivity.this.startActivity(intent);
                        BdjResultActivity.this.finish();
                        return;
                    }
                    ((TextView) BdjResultActivity.this.findViewById(R.id.bdj_rebate_time)).setText("" + remainedSeconds);
                }
            };
            BdjResultActivity.this.executeGUIAction(false, guiAction, new Object[]{Integer.valueOf(forwardSeconds), Integer.valueOf(remainedSeconds)});
        }
    };
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.my_activity_bdj_result);
        backgroundToActivity();
        initView();
        ((Button) findViewById(R.id.bdj_rebate_end)).setOnClickListener(new OnClickListener() {
    ...
    protected void onStart() {
        ...
        try {
            ...
            if (hsmpStatus != null) {
                ...
                int CARD_STATUS = CardEntity.CARD_STATUS;
                try {
                ...
                if (CARD_STATUS == -1) {
                    this.bdjRebateInfoLayout.setVisibility(View.GONE);
                    this.bdjRebateFailedLayout.setVisibility(View.VISIBLE);
                    this.rechargeFailedText.setText(R.string.jywPhoneBlack_lv);
                } else if (CARD_STATUS == -2) {
                    this.bdjRebateInfoLayout.setVisibility(View.GONE);
                    this.bdjRebateFailedLayout.setVisibility(View.VISIBLE);
                    this.rechargeFailedText.setText(R.string.jywPhoneNoBang_lv);
                    ...
                    if (this.isPlaySounds && this.soundPool != null) {
                        this.fanLiWanCheng = this.soundPool.load(this, R.raw.fanliyiwancheng, 0);
                        this.soundPool.setOnLoadCompleteListener(new OnLoadCompleteListener() {
                            public void onLoadComplete(SoundPool soundPool, int sampleId, int status) {
                                if (sampleId == BdjResultActivity.this.fanLiWanCheng) {
                    ...
                    if (!StringUtils.isBlank(jiFen)) {
                        View jiFenInfo = findViewById(R.id.all_jifen_info);
                        jiFenInfo.setVisibility(View.VISIBLE);
                        ((TextView) jiFenInfo.findViewById(R.id.integration)).setText(jiFen);
                    }
                    double rebatethis = 0.0d;
                    try {
                        rebatethis = (Double.parseDouble((String) hsmpStatus.get("RECHARGE")) / 100.0d) * ((double) Integer.parseInt(SysConfig.get("LOCAL_EXCHANGE_RATE")));
                    } catch (Exception e2) {
                        e2.printStackTrace();
                    }
                    this.rebateThis.setText("" + df.format(rebatethis));
                    this.totalMoney = "" + df.format(rebatethis);
                }
            } else {
                this.bdjRebateInfoLayout.setVisibility(View.GONE);
                this.bdjRebateFailedLayout.setVisibility(View.VISIBLE);
    ...
    public void initView() {
        this.rebateThis = (TextView) findViewById(R.id.bdj_rebate_this);
        this.balanceOfINCOM = (TextView) findViewById(R.id.bdj_INCOM_balance);
        this.recharge_warningtext = (TextView) findViewById(R.id.bdj_rebate_remind_text);
        this.bdjRebateInfoLayout = findViewById(R.id.bdj_info);
        this.bdjRebateFailedLayout = findViewById(R.id.bdj_failed_layout);
        this.rechargeFailedText = (TextView) findViewById(R.id.bdj_failed_textview);
        this.bdj_tv_show = (TextView) findViewById(R.id.bdj_tv_show);
