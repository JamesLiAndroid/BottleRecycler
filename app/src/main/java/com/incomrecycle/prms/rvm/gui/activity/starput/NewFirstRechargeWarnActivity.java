...
public class NewFirstRechargeWarnActivity extends BaseActivity {
    private String JSON;
    private int TODAY_BOTTLE = 0;
    private int kaWeiBangDing;
            ...
            GUIAction guiAction = new GUIAction() {
                protected void doAction(Object[] paramObjs) {
                    ...
                    if (remainedSeconds == 0) {
                        ...
                        intent.setClass(NewFirstRechargeWarnActivity.this, EnvironmentalPromotionalActivity.class);
                        intent.putExtra("JSON", NewFirstRechargeWarnActivity.this.JSON);
                        intent.setFlags(FLAG_ACTIVITY_REORDER_TO_FRONT);
                        NewFirstRechargeWarnActivity.this.startActivity(intent);
                        NewFirstRechargeWarnActivity.this.finish();
                        return;
                    }
                    ((TextView) NewFirstRechargeWarnActivity.this.findViewById(R.id.onecard_firt_recharge_time)).setText("" + remainedSeconds);
                }
            };
            NewFirstRechargeWarnActivity.this.executeGUIAction(false, guiAction, new Object[]{Integer.valueOf(forwardSeconds), Integer.valueOf(remainedSeconds)});
        }
    };
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_newonecardfirstrecharge_info);
        backgroundToActivity();
        ((Button) findViewById(R.id.onecard_firt_recharge_bangding)).setOnClickListener(new OnClickListener() {
            public void onClick(View arg0) {
                Intent intent = new Intent(NewFirstRechargeWarnActivity.this, BindCardActivity.class);
                intent.putExtra("JSON", NewFirstRechargeWarnActivity.this.JSON);
                intent.setFlags(FLAG_ACTIVITY_REORDER_TO_FRONT);
                NewFirstRechargeWarnActivity.this.startActivity(intent);
                NewFirstRechargeWarnActivity.this.finish();
        ...
        ((Button) findViewById(R.id.onecard_firt_recharge_next)).setOnClickListener(new OnClickListener() {
            public void onClick(View arg0) {
                Intent intent = new Intent();
                intent.setClass(NewFirstRechargeWarnActivity.this, EnvironmentalPromotionalActivity.class);
                intent.putExtra("JSON", NewFirstRechargeWarnActivity.this.JSON);
    ...
    protected void onStart() {
        super.onStart();
        TextView mneyRecharge = (TextView) findViewById(R.id.onecard_firt_recharge_rebate_this);
        TextView moneyInsideCard = (TextView) findViewById(R.id.onecard_firt_recharge_balance);
        View rechargeLayout = findViewById(R.id.onecard_firt_recharge_rebate_info);
        View insidecardLayout = findViewById(R.id.onecard_firt_recharge_balance_info);
        boolean isPlaySounds = Boolean.parseBoolean(SysConfig.get("IS_PLAY_SOUNDS"));
        if (isPlaySounds && this.soundPool == null) {
            this.soundPool = new SoundPool(1, 3, 0);
        }
        if (isPlaySounds && this.soundPool != null) {
            this.kaWeiBangDing = this.soundPool.load(this, R.raw.kaweibangding, 0);
            this.soundPool.setOnLoadCompleteListener(new OnLoadCompleteListener() {
                public void onLoadComplete(SoundPool soundPool, int sampleId, int status) {
                    if (sampleId == NewFirstRechargeWarnActivity.this.kaWeiBangDing) {
        ...
        try {
            ...
            HashMap<String, Object> hsmpStatus = JSONUtils.toHashMap(this.JSON);
            if (hsmpStatus != null) {
                ...
                try {
                    RECHARGE = (Double.parseDouble((String) hsmpStatus.get("RECHARGE")) / 100.0d) * ((double) Integer.parseInt(SysConfig.get("LOCAL_EXCHANGE_RATE")));
                } catch (Exception e) {
                    e.printStackTrace();
                }
                mneyRecharge.setText("" + df.format(RECHARGE));
            ...
            HashMap<String, Object> hashMapQueryOneCardBalance = guiCommonService.execute("GUITrafficCardCommonService", "queryOneCardBalance", null);
            if (hashMapQueryOneCardBalance != null) {
                String oneCardBalance = (String) hashMapQueryOneCardBalance.get("Balance");
                if (!StringUtils.isBlank(oneCardBalance)) {
                    if (Pattern.compile(SysConfig.get("VERIFY.MONEYINSIDECARD.PATTERN")).matcher(oneCardBalance).matches()) {
                        ...
                        insidecardLayout.setVisibility(View.GONE);
                    }
                }
            }
        } catch (Exception e2) {
    ...
    public void onStop() {
        ...
        findViewById(R.id.onecard_firt_recharge_layout).setBackgroundDrawable(null);
        ((TextView) findViewById(R.id.onecard_firt_recharge_time)).setText("");
