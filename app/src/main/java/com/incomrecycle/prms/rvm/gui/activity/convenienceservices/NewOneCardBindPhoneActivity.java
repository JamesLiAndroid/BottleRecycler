...
public class NewOneCardBindPhoneActivity extends BaseActivity implements OnClickListener {
    ...
    TextWatcher mTextWatcher = new TextWatcher() {
        ...
        public void onTextChanged(CharSequence s, int arg1, int arg2, int arg3) {
            if (this.temp.length() == 0) {
                NewOneCardBindPhoneActivity.this.iv_delete.setVisibility(View.GONE);
            } else {
                NewOneCardBindPhoneActivity.this.iv_delete.setVisibility(View.VISIBLE);
            }
        }

        public void afterTextChanged(Editable s) {
            this.editStart = NewOneCardBindPhoneActivity.this.phoneNumEdit.getSelectionStart();
            this.editEnd = NewOneCardBindPhoneActivity.this.phoneNumEdit.getSelectionEnd();
        }
    };
    ...
    private TimeoutAction timeoutAction = new TimeoutAction() {
        public void apply(int forwardSeconds, int remainedSeconds) {
            GUIAction guiAction = new GUIAction() {
                protected void doAction(Object[] paramObjs) {
                    ...
                    if (remainedSeconds != 0) {
                        ((TextView) NewOneCardBindPhoneActivity.this.findViewById(R.id.query_onecard_bind_card_time)).setText("" + remainedSeconds);
                    } else if (!StringUtils.isBlank(SysConfig.get("RVMMActivity.class"))) {
                        try {
                            Intent intent = new Intent(NewOneCardBindPhoneActivity.this.getBaseContext(), Class.forName(SysConfig.get("RVMMActivity.class")));
                            intent.setFlags(FLAG_ACTIVITY_REORDER_TO_FRONT);
                            NewOneCardBindPhoneActivity.this.startActivity(intent);
                            NewOneCardBindPhoneActivity.this.finish();
                        } catch (Exception e) {
                            e.printStackTrace();
                        }
                    }
            ...
            NewOneCardBindPhoneActivity.this.executeGUIAction(false, guiAction, new Object[]{Integer.valueOf(forwardSeconds), Integer.valueOf(remainedSeconds)});
        }
    };
        ...
        if (this.isPlaySounds && this.soundPool != null) {
            ...
            this.soundPool.setOnLoadCompleteListener(new OnLoadCompleteListener() {
                public void onLoadComplete(SoundPool soundPool, int sampleId, int status) {
                    if (sampleId == NewOneCardBindPhoneActivity.this.bangDing) {
    ...
    public void finish() {
        super.finish();
        ((TextView) findViewById(R.id.query_onecard_bind_card_time)).setText("");
            ...
            case R.id.query_onecard_bind_card_confirm:
                if (!StringUtils.isBlank(this.phoneNumEdit.getText().toString())) {
                    ...
                    if ("none".equalsIgnoreCase(result)) {
                        ...
                        try {
                            HashMap<String, Object> hsmpRet = guiCommonService.execute("GUITrafficCardCommonService", "bindOneCard", hp);
                            if (hsmpRet != null) {
