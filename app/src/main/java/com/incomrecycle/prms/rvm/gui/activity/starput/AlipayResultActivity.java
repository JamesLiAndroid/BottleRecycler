...
public class AlipayResultActivity extends BaseActivity {
            ...
            GUIAction guiAction = new GUIAction() {
                protected void doAction(Object[] paramObjs) {
                    ...
                    if (remainedSeconds == 0) {
                        ...
                        if (!StringUtils.isBlank(AlipayResultActivity.this.JSON) && "BOTTLE".equalsIgnoreCase(ServiceGlobal.getCurrentSession("PRODUCT_TYPE").toString())) {
                            intent.setClass(AlipayResultActivity.this, EnvironmentalPromotionalActivity.class);
                            intent.putExtra("JSON", AlipayResultActivity.this.JSON);
                        } else if (!StringUtils.isBlank(SysConfig.get("RVMMActivity.class"))) {
                            try {
                                intent.setClass(AlipayResultActivity.this.getBaseContext(), Class.forName(SysConfig.get("RVMMActivity.class")));
                            } catch (Exception e) {
                                e.printStackTrace();
                            }
                        }
                    ...
                    ((TextView) AlipayResultActivity.this.findViewById(R.id.alipay_rebate_time)).setText("" + remainedSeconds);
                }
            };
            AlipayResultActivity.this.executeGUIAction(false, guiAction, new Object[]{Integer.valueOf(forwardSeconds), Integer.valueOf(remainedSeconds)});
        }
    };
        ...
        try {
            ...
            if (hsmpStatus != null) {
                ...
                if (CARD_STATUS == -1 || CARD_STATUS == -2) {
                    this.recharge_warningtext.setText(R.string.alipay_unuse);
                    this.alipayRebateInfoLayout.setVisibility(View.GONE);
                } else {
                    if (this.isPlaySounds && this.soundPool != null) {
                        ...
                        this.soundPool.setOnLoadCompleteListener(new OnLoadCompleteListener() {
                            public void onLoadComplete(SoundPool soundPool, int sampleId, int status) {
                                if (sampleId == AlipayResultActivity.this.fanLiWanCheng) {
                ...
                this.recharge_warningtext.setText(R.string.commit_rebateinfo_fail_text_bdj);
                this.alipayRebateInfoLayout.setVisibility(View.GONE);
            }
    ...
    public void initView() {
        this.rebateThis = (TextView) findViewById(R.id.alipay_rebate_this);
        this.balanceOfINCOM = (TextView) findViewById(R.id.alipay_INCOM_balance);
        this.recharge_warningtext = (TextView) findViewById(R.id.alipay_rebate_remind_text);
        this.alipayRebateInfoLayout = findViewById(R.id.alipay_info);
    }
    ...
    private void showRemindInfo() {
        if (!"FALSE".equalsIgnoreCase(SysConfig.get("BOTTLES_LIMITED_ENABLE"))) {
            ...
            if (this.TODAY_BOTTLE == 0) {
                ...
                remind = StringUtils.replace(StringUtils.replace(StringUtils.replace(StringUtils.replace(getString(R.string.put_bottle_max_warn),
                        "$MAX_REBATE_BOTTLES_NUM$", BOTTLES_DAILY_ALARM_SET), "$TODAY_PUT_BOTTLE_NUM$", todayBottleCount + ""),
                        "$PUT_BOTTLE_NUM$", THIS_BOTTLE_COUNT + ""), "$PUT_BOTTLE_MONEY$", this.totalMoney);
            }
