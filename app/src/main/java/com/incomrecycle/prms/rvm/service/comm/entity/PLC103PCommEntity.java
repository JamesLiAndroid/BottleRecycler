...
public class PLC103PCommEntity implements CommEntity {
    private static final long BUTTON_TIME_INTERVAL = 1000;
    private static final Logger logger = LoggerFactory.getLogger("PLC");
    ...
    private long lPaperDoorSwitchingTime;
    ...
    private int paperDoorState;
    private int paperLightState;
    private int paperRecycleFinishState;
    ...
    private class ExecuteThread extends Thread {
        ...
        public void run() {
            while (true) {
                byte[] data = (byte[]) PLC103PCommEntity.this.plcResponseFIFOQueue.pop();
                if (data != null) {
                    try {
                        PLC103PCommEntity.this.execute(data);
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
                } else {
            ...
            try {
                if ("TRUE".equalsIgnoreCase(SysConfig.get("COM.PLC.HAS.DOOR"))) {
                    if ("OPEN".equalsIgnoreCase(SysConfig.get("COM.PLC.DOOR.STATE.INIT"))) {
                        PLC103PCommEntity.this.sendComm(PLC_CMD.DOOR_OPEN);
                        PLC103PCommEntity.this.doorState = 1;
                    } else {
                        PLC103PCommEntity.this.sendComm(PLC_CMD.DOOR_CLOSE);
                        PLC103PCommEntity.this.doorState = 0;
                    }
                } else {
                    PLC103PCommEntity.this.doorState = 1;
                    PLC103PCommEntity.this.currentDoorState = 1;
                }
                Thread.sleep(200);
                if (PLC103PCommEntity.this.hasLightCmd) {
                    PLC103PCommEntity.this.sendComm(PLC_CMD.LIGHT_OFF);
                }
                Thread.sleep(200);
                PLC103PCommEntity.this.sendComm(PLC_CMD.BELT_FORWARD);
                Thread.sleep(5000);
                PLC103PCommEntity.this.sendComm(PLC_CMD.BELT_STOP);
                PLC103PCommEntity.this.sendComm(PLC_CMD.PAPER_MACHINE_FORWARD);
                PLC103PCommEntity.this.beltState = 0;
                PLC103PCommEntity.this.execute("RVM_CLOCK_CALIBRATION", null);
                ...
                if (StringUtils.isBlank(POWER_ON_TIME) || StringUtils.isBlank(POWER_OFF_TIME)) {
                    PLC103PCommEntity.this.execute("RVM_POWER_OFF_DISABLE", null);
                    return;
                }
                HashMap hsmpParam = new HashMap();
                hsmpParam.put("POWER_ON_TIME", POWER_ON_TIME);
                hsmpParam.put("POWER_OFF_TIME", POWER_OFF_TIME);
                PLC103PCommEntity.this.execute("RVM_POWER_OFF_ENABLE", JSONUtils.toJSON(hsmpParam));
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
    ...
    private static final class PLC_CMD {
        ...
        private static final byte[] CHECK_THIRD_LIGHT_STATE = "EEEE".getBytes();
        ...
        private static final byte[] PAPER_BARCODE_LIGHT_OFF = "LLLL".getBytes();
        private static final byte[] PAPER_BARCODE_LIGHT_ON = "IIII".getBytes();
        private static final byte[] PAPER_DOOR_CLOSE = "HHHH".getBytes();
        private static final byte[] PAPER_DOOR_OPEN = "PPPP".getBytes();
        private static final byte[] PAPER_LIGHT_FLASH = "DDDD".getBytes();
        private static final byte[] PAPER_LIGHT_OFF = "FFFF".getBytes();
        private static final byte[] PAPER_LIGHT_ON = "BBBB".getBytes();
        private static final byte[] PAPER_MACHINE_FORWARD = "XXXX".getBytes();
        private static final byte[] PAPER_STORAGE_DOOR_OPEN = "TTTT".getBytes();
        private static final byte[] PAPER_WEIGH_RESET = "CCCC".getBytes();
    ...
    private static final class PaperLightState {
        private static final int PAPER_LIGHT_OFF = 0;
        private static final int PAPER_LIGHT_ON = 1;

        private PaperLightState() {
        }
    }

    private static final class PaperRecycleState {
        private static final int FINISH = 1;
        private static final int UNFINISH = 0;

        private PaperRecycleState() {
        }
    }
    ...
    private static final class StateMask {
        ...
        private static final int JUDGEMENT = 255;
        private static final int PAPER_DOOR_CLOSE = 2;
        private static final int PAPER_DOOR_OPEN = 1;
    ...
    private class PLCCommStateCallBack implements CommStateCallBack {
        ...
        public void apply(byte[] data) {
            ...
            if (doorCloseSignal != 0) {
                PLC103PCommEntity.this.currentDoorState = 0;
            }
            ...
            if (PLC103PCommEntity.this.lightCheck) {
            ...
            if ("TRUE".equalsIgnoreCase(SysConfig.get("RVM.QUERY.LIGHT.STATE")) && PLC103PCommEntity.this.lightCheck) {
                ...
                if (firstLightSignal > 0) {
                    ...
                    if (this.firstLightNoErrorTimes == this.maxErrorTimes) {
                        firstLightCheckedChanged = true;
                        PLC103PCommEntity.this.doorCloseTryTimes = 0;
                    }
                }
                    ...
                    do {
                        ...
                        if (event != null) {
                            ...
                            if ("CMD:STOP".equals(event)) {
                                ...
                                step = 8;
                            } else if ("EVENT:SECOND_LIGHT_ON".equals(event) || "EVENT:BARCODE_READED".equals(event)) {
                                step = 4;
                                delayTime = 0;
                            }
                        }
            ...
            try {
                ...
                if (this.machineComm == null) {
                    ...
                    SysGlobal.execute(new Thread() {
                        public void run() {
                            try {
                                PLC103PCommEntity.this.PLCAuto();
                            } catch (Exception e) {
                                e.printStackTrace();
                            }
                        }
    ...
    public static String getCmdName(byte[] cmd) {
        ...
        if (cmd == PLC_CMD.PAPER_DOOR_OPEN) {
            return DoorStatus.PAPER_DOOR_OPEN;
        }
        if (cmd == PLC_CMD.PAPER_DOOR_CLOSE) {
            return DoorStatus.PAPER_DOOR_CLOSE;
        }
        if (cmd == PLC_CMD.PAPER_MACHINE_FORWARD) {
            return "PAPER_MACHINE_FORWARD";
        }
        if (cmd == PLC_CMD.PAPER_WEIGH_RESET) {
            return "PAPER_WEIGH_RESET";
        }
        if (cmd == PLC_CMD.PAPER_LIGHT_ON) {
            return "PAPER_LIGHT_ON";
        }
        if (cmd == PLC_CMD.PAPER_LIGHT_OFF) {
            return "PAPER_LIGHT_OFF";
        }
        if (cmd == PLC_CMD.PAPER_LIGHT_FLASH) {
            return "PAPER_LIGHT_FLASH";
        }
        if (cmd == PLC_CMD.PAPER_STORAGE_DOOR_OPEN) {
            return "PAPER_STORAGE_DOOR_OPEN";
        }
        if (cmd == PLC_CMD.PAPER_BARCODE_LIGHT_ON) {
            return "PAPER_BARCODE_LIGHT_ON";
        }
        if (cmd == PLC_CMD.PAPER_BARCODE_LIGHT_OFF) {
            return "PAPER_BARCODE_LIGHT_OFF";
        }
    ...
    public PLC103PCommEntity() {
        ...
        this.paperDoorState = 0;
        this.paperLightState = 0;
        this.isToThanks = false;
        this.beltState = 0;
        this.doorState = 0;
        ...
        this.currentPaperDoorState = 0;
        this.paperRecycleFinishState = 0;
        this.forceClosePaperDoor = false;
        this.lPaperDoorSwitchingTime = 0;
        ...
        this.taskActionMonitor = new TaskAction() {
            public void execute() {
                PLC103PCommEntity.this.checkPlcError();
                if (PLC103PCommEntity.this.doorCloseTryTimes < PLC103PCommEntity.this.MAX_DOORCLOSE_TRYTIMES && !PLC103PCommEntity.this.isInCheck && PLC103PCommEntity.this.hasPlcDoor && PLC103PCommEntity.this.doorState == 0 && PLC103PCommEntity.this.lWaitDoorChangedTime > 0 && PLC103PCommEntity.this.currentDoorState == 1 && PLC103PCommEntity.this.lWaitDoorChangedTime + Long.parseLong(SysConfig.get("COM.PLC.DOOR.TIMEOUT")) < System.currentTimeMillis()) {
                    PLC103PCommEntity.this.lWaitDoorChangedTime = System.currentTimeMillis();
                    PLC103PCommEntity.this.doorErrorTimes = PLC103PCommEntity.this.doorErrorTimes + 1;
                    PLC103PCommEntity.this.doorCloseTryTimes = PLC103PCommEntity.this.doorCloseTryTimes + 1;
                    try {
                        PLC103PCommEntity.this.sendComm(PLC_CMD.DOOR_CLOSE);
                    } catch (Exception e) {
                        e.printStackTrace();
                ...
                if (!PLC103PCommEntity.this.isInCheck && PLC103PCommEntity.this.hasPlcDoor && PLC103PCommEntity.this.doorState == 1 && PLC103PCommEntity.this.lWaitDoorChangedTime > 0 && PLC103PCommEntity.this.currentDoorState == 0 && PLC103PCommEntity.this.lWaitDoorChangedTime + Long.parseLong(SysConfig.get("COM.PLC.DOOR.TIMEOUT")) < System.currentTimeMillis()) {
                    PLC103PCommEntity.this.lWaitDoorChangedTime = System.currentTimeMillis();
                    PLC103PCommEntity.this.doorErrorTimes = PLC103PCommEntity.this.doorErrorTimes + 1;
                    try {
                        PLC103PCommEntity.this.sendComm(PLC_CMD.DOOR_OPEN);
                ...
                if (PLC103PCommEntity.this.lPaperDoorSwitchingTime > 0 && PLC103PCommEntity.this.paperDoorState == 1 && PLC103PCommEntity.this.currentPaperDoorState == 0 && PLC103PCommEntity.this.lPaperDoorSwitchingTime + Long.parseLong(SysConfig.get("COM.PLC.PAPERDOOR.TIMEOUT")) < System.currentTimeMillis()) {
                    PLC103PCommEntity.this.lPaperDoorSwitchingTime = System.currentTimeMillis();
                    try {
                        PLC103PCommEntity.this.execute("FORCE_PAPER_DOOR_OPEN", null);
                    } catch (Exception e22) {
                        e22.printStackTrace();
                    }
                }
                if (PLC103PCommEntity.this.lWaitBeltStopTime > 0) {
                    if (PLC103PCommEntity.this.beltState != 0 && PLC103PCommEntity.this.lWaitBeltStopTime + (Long.parseLong(SysConfig.get("COM.PLC.BELTROLLING.TIMEOUT")) * PLC103PCommEntity.BUTTON_TIME_INTERVAL) < System.currentTimeMillis()) {
                        try {
                            PLC103PCommEntity.this.sendCmd(PLC_CMD.BELT_STOP);
                            PLC103PCommEntity.this.lWaitBeltStopTime = 0;
                        } catch (Exception e222) {
                            e222.printStackTrace();
                        }
                    }
                    if (PLC103PCommEntity.this.beltState == 1 && System.currentTimeMillis() - PLC103PCommEntity.this.lBeltRollingContinueTime > PLC103PCommEntity.BUTTON_TIME_INTERVAL && PLC103PCommEntity.this.lWaitBeltStopTime + (Long.parseLong(SysConfig.get("COM.PLC.BELTROLLING.TIMEOUT")) * PLC103PCommEntity.BUTTON_TIME_INTERVAL) > System.currentTimeMillis()) {
                        try {
                            PLC103PCommEntity.this.lBeltRollingContinueTime = System.currentTimeMillis();
        ...
        this.commRecvCallBack = new CommRecvCallBack() {
            public void apply(byte[] data) {
                PLC103PCommEntity.this.plcResponseFIFOQueue.push(data);
            }
        };
                ...
                SysGlobal.execute(new Thread() {
                    ...
                    public void run() {
                        try {
                            ...
                            if (this.beltState == PLC103PCommEntity.this.beltState) {
                                PLC103PCommEntity.this.sendComm(this.cmd);
                                PLC103PCommEntity.this.lBeltStateTime = System.currentTimeMillis();
                                PLC103PCommEntity.this.lLastCmdTime = System.currentTimeMillis();
                            }
                        } catch (Exception e) {
                            e.printStackTrace();
                        }
                ...
                if (!this.isInCheck) {
                    ...
                    if ("RESET".equalsIgnoreCase(cmd)) {
                        ...
                        if (this.heartBeatEnable && "HEART_BEAT".equalsIgnoreCase(cmd)) {
                            ...
                            buffer = new byte[]{MachineComm.TimeControl.ALIVE_TIME, (byte) ((rvm_alive_time >> 16) & MotionEventCompat.ACTION_MASK), (byte) ((rvm_alive_time >> 8) & MotionEventCompat.ACTION_MASK), (byte) (rvm_alive_time & MotionEventCompat.ACTION_MASK)};
                            ...
                            if ("PAPER_STORAGE_DOOR_OPEN".equalsIgnoreCase(cmd)) {
                                sendCmd(PLC_CMD.PAPER_STORAGE_DOOR_OPEN);
                                return null;
                            } else if ("FORCE_PAPER_DOOR_OPEN".equalsIgnoreCase(cmd)) {
                                this.lPaperDoorSwitchingTime = System.currentTimeMillis();
                                this.paperDoorState = 1;
                                sendCmd(PLC_CMD.PAPER_DOOR_OPEN);
                                return null;
                            } else if ("PAPER_WEIGH_RESET".equalsIgnoreCase(cmd)) {
                                sendCmd(PLC_CMD.PAPER_WEIGH_RESET);
                                return null;
                            } else if (DoorStatus.PAPER_DOOR_OPEN.equalsIgnoreCase(cmd)) {
                                if (this.paperDoorState != 1) {
                                    this.paperDoorState = 1;
                                    this.lPaperDoorSwitchingTime = System.currentTimeMillis();
                                    sendCmd(PLC_CMD.PAPER_DOOR_OPEN);
                                }
                                if (this.paperLightState != 1) {
                                    sendCmd(PLC_CMD.PAPER_LIGHT_ON);
                                    this.paperLightState = 1;
                                }
                                return null;
                            } else if (DoorStatus.PAPER_DOOR_CLOSE.equalsIgnoreCase(cmd) && "FORCE_CLOSE_DOOR".equalsIgnoreCase(json)) {
                                this.paperDoorState = 0;
                                this.lPaperDoorSwitchingTime = System.currentTimeMillis();
                                sendCmd(PLC_CMD.PAPER_DOOR_CLOSE);
                                if (this.paperLightState != 0) {
                                    sendCmd(PLC_CMD.PAPER_LIGHT_OFF);
                                    this.paperLightState = 0;
                                }
                                this.forceClosePaperDoor = true;
                                return null;
                            } else if (DoorStatus.PAPER_DOOR_CLOSE.equalsIgnoreCase(cmd) && "RECYCLE_PAPER_FINISH".equalsIgnoreCase(json)) {
                                if (this.paperDoorState != 0) {
                                    this.paperDoorState = 0;
                                    this.lPaperDoorSwitchingTime = System.currentTimeMillis();
                                    sendCmd(PLC_CMD.PAPER_DOOR_CLOSE);
                                }
                                ...
                                this.paperRecycleFinishState = 1;
                                return null;
                            } else if ("INFORM_ENTITY_TO_THANKS".equalsIgnoreCase(cmd)) {
                                this.isToThanks = true;
                                if (this.currentPaperDoorState != 0) {
                                    HashMap<String, String> hsmpParam2 = new HashMap();
                                    hsmpParam2.put("type", "PAPER_RECYCLE_FINISH_DOOR_CLOSE");
                                    ServiceGlobal.getCommEventQueye().push(hsmpParam2);
                                }
                                return null;
                            } else if ("RECYCLE_PAPER".equalsIgnoreCase(cmd)) {
                                sendCmd(PLC_CMD.PAPER_MACHINE_FORWARD);
                                return null;
                            } else if ("CHECK_PAPER_DOOR_STATE".equalsIgnoreCase(cmd)) {
                                if (this.currentPaperDoorState == 0) {
                                    str = DoorStatus.PAPER_DOOR_CLOSE;
                                    return str;
                                }
                                str = DoorStatus.PAPER_DOOR_OPEN;
                                return str;
                            } else if ("PAPER_BARCODE_LIGHT_ON".equalsIgnoreCase(cmd)) {
                                sendCmd(PLC_CMD.PAPER_BARCODE_LIGHT_ON);
                                return null;
                            } else if ("PAPER_BARCODE_LIGHT_OFF".equalsIgnoreCase(cmd)) {
                                sendCmd(PLC_CMD.PAPER_BARCODE_LIGHT_OFF);
                                return null;
                            } else {
                                ...
                                if ("RECYCLE_PAPER_END".equalsIgnoreCase(cmd)) {
                                    this.serviceEnable = false;
                                    serviceEnableChange();
                                }
                                if ("QUERY_LIGHT_STATE".equalsIgnoreCase(cmd)) {
                                ...
                                if ("SET_SECOND_LIGHT_ON".equalsIgnoreCase(cmd)) {
                                    SysGlobal.execute(new Runnable() {
                                        public void run() {
                                            try {
                                                Thread.sleep(1500);
                                                PLC103PCommEntity.this.eventFIFOQueue.push("EVENT:BARCODE_READED");
                    ...
                    return null;
                } else if ("PLC:PAPER_DOOR_OPEN".equalsIgnoreCase(cmd)) {
                    sendCmd(PLC_CMD.PAPER_DOOR_OPEN);
                    return null;
                } else if ("PLC:PAPER_DOOR_CLOSE".equalsIgnoreCase(cmd)) {
                    sendCmd(PLC_CMD.PAPER_DOOR_CLOSE);
                    return null;
                } else if ("PLC:RECYCLE_PAPER".equalsIgnoreCase(cmd)) {
                    sendCmd(PLC_CMD.PAPER_MACHINE_FORWARD);
                    return null;
                } else if ("PLC:PAPER_WEIGH_RESET".equalsIgnoreCase(cmd)) {
                    sendCmd(PLC_CMD.PAPER_WEIGH_RESET);
                    return null;
                } else if ("PLC:PAPER_LIGHT_ON".equalsIgnoreCase(cmd)) {
                    sendCmd(PLC_CMD.PAPER_LIGHT_ON);
                    return null;
                } else if ("PLC:PAPER_LIGHT_OFF".equalsIgnoreCase(cmd)) {
                    sendCmd(PLC_CMD.PAPER_LIGHT_OFF);
                    return null;
                } else {
                    return null;
    ...
    private void execute(byte[] buffer) throws Exception {
        ...
        int paperDoorOpenSignal = buffer[2] & 1;
        int paperDoorCloseSignal = buffer[2] & 2;
        ...
        if (secondLightSignal <= 0 || thirdLightSignal <= 0) {
            if (paperDoorOpenSignal > 0 || paperDoorCloseSignal > 0) {
                int flag2 = buffer[1] & MotionEventCompat.ACTION_MASK;
                if ((buffer[0] & MotionEventCompat.ACTION_MASK) > 0 || flag2 > 0) {
                    paperDoorOpenSignal = 0;
                    paperDoorCloseSignal = 0;
                }
            }
            long lTime = System.currentTimeMillis();
            if (firstLightSignal > 0) {
            ...
            if (this.isInCheck) {
                ...
                if (paperDoorOpenSignal > 0) {
                    hsmpParam = new HashMap();
                    hsmpParam.put("type", "CHECK");
                    hsmpParam.put("comm", "PLC");
                    hsmpParam.put("msg", DoorStatus.PAPER_DOOR_OPEN);
                    ServiceGlobal.getCommEventQueye().push(hsmpParam);
                }
                if (paperDoorCloseSignal > 0) {
                    hsmpParam = new HashMap();
                    hsmpParam.put("type", "CHECK");
                    hsmpParam.put("comm", "PLC");
                    hsmpParam.put("msg", DoorStatus.PAPER_DOOR_CLOSE);
                    ServiceGlobal.getCommEventQueye().push(hsmpParam);
                    return;
                }
                return;
            }
            if (buttonSignal == 0 && firstLightSignal == 0 && secondLightSignal == 0 && thirdLightSignal == 0 && doorOpenSignal == 0
                    && doorCloseSignal == 0 && paperDoorOpenSignal == 0 && paperDoorCloseSignal == 0) {
            ...
            if (secondLightSignal > 0) {
                ...
                if (this.secondLightDelay <= 0) {
                    ...
                    SysGlobal.execute(new Thread() {
                        public void run() {
                            try {
                                Thread.sleep(PLC103PCommEntity.this.secondLightDelay);
                            } catch (Exception e) {
                            }
                            PLC103PCommEntity.this.eventFIFOQueue.push("EVENT:SECOND_LIGHT_ON");
                        }
                    });
                }
            }
            ...
            if (paperDoorOpenSignal != 0) {
                this.currentPaperDoorState = 1;
            }
            if (paperDoorCloseSignal != 0) {
                this.currentPaperDoorState = 0;
            }
            if (paperDoorCloseSignal > 0 && this.paperRecycleFinishState == 1) {
                if (!this.isToThanks) {
                    hsmpParam = new HashMap();
                    hsmpParam.put("type", "PAPER_RECYCLE_FINISH_DOOR_CLOSE");
                    hsmpParam.put("data", "PAPER_RECYCLE_FINISH_DOOR_CLOSE");
                    ServiceGlobal.getCommEventQueye().push(hsmpParam);
                }
                this.currentPaperDoorState = 0;
                this.paperRecycleFinishState = 0;
            }
            if (paperDoorCloseSignal > 0 && this.forceClosePaperDoor) {
                this.currentPaperDoorState = 0;
                this.forceClosePaperDoor = false;
                this.paperRecycleFinishState = 0;
                return;
    ...
    private void sendComm(byte[] cmd) throws Exception {
        if (cmd[0] == (byte) -120 || cmd[0] == MachineComm.TimeControl.TIME_POWER_ON ||
                ...
                cmd[0] == MachineComm.TimeControl.TIME_ENABLE || cmd[0] == MachineComm.TimeControl.ALIVE_TIME ||
                cmd[0] == MachineComm.TimeControl.ALIVE_TIME_MAXTIMES) {
