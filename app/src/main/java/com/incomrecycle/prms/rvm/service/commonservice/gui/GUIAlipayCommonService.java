...
public class GUIAlipayCommonService implements AppCommonService {
        ...
        try {
            ...
            if (TRANSMIT_ADV == null || TRANSMIT_ADV.size() <= 0) {
                ...
                if (mapVendingWayFlag != null && mapVendingWayFlag.size() > 0) {
                    ...
                    if (VENDING_WAY_SET == null || !VENDING_WAY_SET.contains("ALIPAY")) {
                        String VENDING_WAY = (String) mapVendingWayFlag.get(AllAdvertisement.VENDING_WAY);
                        if (VENDING_WAY != null && "ALIPAY".equalsIgnoreCase(VENDING_WAY)) {
            ...
            sqlUpdateBuilder.setString(AllAdvertisement.VENDING_WAY, "ALIPAY").setNumber("OPT_STATUS", Integer.valueOf(1)).setString("CARD_TYPE", CARD_TYPE).setString("CARD_NO", CARD_NO).setString(AllAdvertisement.SELECT_FLAG, SELECT_FLAG).setSqlWhere(sqlWhereBuilderRvmOpt);
    ...
    private HashMap verifyQRCode(String svcName, String subSvcName, HashMap hsmpParam) {
        ...
        } else if (QRCODE_NO.startsWith("TB_")) {
            ServiceGlobal.setCurrentSession("CARD_NO", QRCODE_NO);
            ServiceGlobal.setCurrentSession("CARD_TYPE", "ALIPAY");
